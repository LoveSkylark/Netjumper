...
from tree_utils import CLITreeBuilder
from saci_models import VLANPoolRange
from config_service import load_apic_settings
...
class ACIClient:
    def __init__(self, apic_url: str, token_file: str = "~/.aci_token", verify_ssl: bool = False, username: str = "", password: str = "") -> None:
    ...
    def login(self):
        ...
        username = self.username or os.environ.get('APIC_USERNAME')
        password = self.password or os.environ.get('APIC_PASSWORD')
    ...
    def handle_aaep_list_command(self):
        ...
        print("\nTip: Use 'saci aaep <name>' to see the connection map for a specific AAEP")
    ...
    def print_tree(tree, label=None):
        ...
        CLITreeBuilder.print_tree(tree, label)
...
def main():
    ...
    apic_settings = load_apic_settings()
    url = apic_settings.url

    if not url:
        logger.error("apic.url is not set in config.yaml")
        logger.info("Please add APIC settings under the 'apic' section in config.yaml")
        logger.info("Example: apic.url: https://apic.example.com")
        exit(1)

    verify_ssl = apic_settings.verify_ssl

    args = parse_args()
    apic = ACIClient(url, verify_ssl=verify_ssl, username=apic_settings.username, password=apic_settings.password)
    apic.login()
